...
namespace mana {
    ...
    public:
        HashMap() = default;
        HashMap(std::initializer_list<std::pair<const K, V>> init) : data_(init) {}

        void insert(const K& key, V value) { data_[key] = std::move(value); }
        ...
        Vec<K> keys() const {
            Vec<K> result;
            for (const auto& p : data_) result.push(p.first);
            return result;
        }

        Vec<V> values() const {
            Vec<V> result;
            for (const auto& p : data_) result.push(p.second);
            return result;
        }
    ...
    public:
        HashSet() = default;
        HashSet(std::initializer_list<T> init) : data_(init) {}

        void insert(T value) { data_.insert(std::move(value)); }
        void remove(const T& value) { data_.erase(value); }
        bool contains(const T& value) const { return data_.count(value) > 0; }
        ...
        Vec<T> to_vec() const {
            Vec<T> result;
            for (const auto& v : data_) result.push(v);
            return result;
        }
    ...
    inline Result<bool, std::string> write_file(const std::string& path, const std::string& content) {
        ...
        if (!file.is_open()) {
            return Result<bool, std::string>::Err("failed to create file: " + path);
        }
        file << content;
    ...
    inline Result<bool, std::string> append_file(const std::string& path, const std::string& content) {
        ...
        if (!file.is_open()) {
            return Result<bool, std::string>::Err("failed to open file for append: " + path);
        }
        file << content;
        return Result<bool, std::string>::Ok(true);
    }
    ...
    template <typename T>
    inline void vec_sort(Vec<T>& vec) {
        std::sort(vec.begin(), vec.end());
    }

    template <typename T>
    inline void vec_reverse(Vec<T>& vec) {
        std::reverse(vec.begin(), vec.end());
    }

    template <typename T>
    inline bool vec_contains(const Vec<T>& vec, const T& value) {
        for (size_t i = 0; i < vec.len(); ++i) {
    ...
    inline std::string format(const std::string& fmt, T first, Args... args) {
        std::string result;
        size_t pos = 0;
        size_t brace = fmt.find("{}", pos);
        if (brace != std::string::npos) {
            result = fmt.substr(0, brace);
            std::ostringstream oss;
            oss << first;
            result += oss.str();
            result += format(fmt.substr(brace + 2), args...);
        } else {
            result = fmt;
        }
        return result;
