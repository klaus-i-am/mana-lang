...
static std::string map_type(const std::string& mana_type) {
    ...
    if (mana_type.front() == '[') {
        ...
        // Format: [count]elem (e.g., [5]i32) - generated by infer_type_name
        size_t close = mana_type.find(']');
        if (close != std::string::npos) {
            std::string count = mana_type.substr(1, close - 1);
            std::string elem = mana_type.substr(close + 1);
            if (!count.empty() && !elem.empty()) {
                return "std::array<" + map_type(elem) + ", " + count + ">";
            }
        }
    }
...
void CppEmitter::emit_expr(const AstExpr* e, std::ostream& out) {
    ...
    switch (e->kind) {
        ...
        case NodeKind::OrExpr: {
            // expr or return/break/{ block }/default_value
            ...
            out << ";\n";
            // Use is_ok()/unwrap() for Result, is_some()/unwrap() for Option
            ...
            } else if (oe->has_default()) {
                // Default value: return the default expression
                out << "        return ";
